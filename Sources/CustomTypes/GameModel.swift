...
public protocol GameModel
  {
    ...
    /// Return the element corresponding to the given affinity, if any.
    func element(for affinity: Affinity) -> Element?

    /// Return the affinity corresponding to the given skill type, if any.
    func affinity(for skillType: SkillType) -> Affinity?
  }


extension GameModel
  {
    /// Return the element corresponding to the given skill type, if any.
    public func element(for skillType: SkillType) -> Element?
      { affinity(for: skillType).flatMap { element(for: $0) } }
  }
