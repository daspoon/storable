...
public class IngestContext
  {
    ...
    public func classInfo(for entityName: String) throws -> Schema.ClassInfo
      {
        guard let entity = dataStore.classInfoByName[entityName] else { throw Exception("unknown entity name '\(entityName)'") }
        return entity
      }
